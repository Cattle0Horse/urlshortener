...
func New(conn *gorm.DB) (TDDL, error) {
	cfg := config.Get().TDDL
	return newSequence(conn, cfg.Step, cfg.SeqName, cfg.StartNum)
}

// newSequence creates a new tddlSequence instance
func newSequence(conn *gorm.DB, step uint64, seqName string, startNum uint64) (*tddlSequence, error) {
	if step < 1 {
	...
	if err := s.getRowID(seqName, startNum); err != nil {
		return nil, err
	}
